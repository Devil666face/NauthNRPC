...
def args_parser():
    parser = argparse.ArgumentParser(
        add_help=True,
        description="NAuthNRPC tool to access MS-NRPC interface methods "
        "under auth level=1."
    ...
    parser.add_argument(
        "-t", "--target", action="store", help="Domain controller IP address"
    )
    parser.add_argument(
        "-u",
        "--usersfile",
        action="store",
        help="File that contains user accounts",
        required=False,
    )
    parser.add_argument(
        "-c",
        "--computersfile",
        action="store",
        help="File that contains computer accounts",
        required=False,
    )
...
class NAuthNRPC:
        ...
        if computers_file is not None:
            try:
                with open(computersfile, "r") as computers:
                    self.computers = [
                        computer.strip() for computer in computers.readlines()
                    ]
            except Exception as e:
    ...
    def domain_info_parser(self):
        ...
        """
        output = {
            "DC Name": self.domain_info["DomainControllerName"][2:],
                ...
                if domain == "DomainSid":
                    if domains[i]["DomainSid"] == b"":
                        ...
                        iden = struct.unpack(
                            ">Q",
                            b"\x00\x00"
                            + domains[i]["DomainSid"]["IdentifierAuthority"],
                        )[0]
                        subauth = "-".join(
                            map(str, domains[i]["DomainSid"]["SubAuthority"])
                        )
                    ...
                    if domains[i][domain] in trust_attribute_mapping:
                        output[i]["Trust Attributes"] = trust_attribute_mapping[
                            domains[i][domain]
                        ]
                    else:
        ...
        for key, value in self.output.items():
            if key == "Trusted Domains":
                if domain_info_printed:
                    ...
                    self.positive_log("[*] Trusted Domains Information\n" + "-" * 30)
                for subkey, subvalue in value.items():
                    print(f"[*] Trusted Domain number {subkey}")
                    self.positive_log(f"[*] Trusted Domain number {subkey}")
                    for ssubkey, ssubvalue in subvalue.items():
                        if not bool(ssubvalue):
                            ssubvalue = "Not Available"
                        print(f"    \u2022 {ssubkey}: {ssubvalue}")
                        self.positive_log(f"    \u2022 {ssubkey}: {ssubvalue}")
            else:
                if not domain_info_printed:
                    print("[*] Domain Information\n" + "-" * 30)
                    self.positive_log("[*] Domain Information\n" + "-" * 30)
                    domain_info_printed = True
                    if not bool(value):
                        value = "Not Available"
                print(f"[*] {key}: {value}")
                self.positive_log(f"[*] {key}: {value}")

    def user_thread(self, username: str):
        if self.domain_info_flag:
            response = hDsrGetDcNameEx2(
                self.dce,
            ...
            response = hDsrGetDcNameEx2(
                self.dce, NULL, username, 0x200, NULL, NULL, NULL, 0
            )

        if response is not None and response["ErrorCode"] == 0:
            print(f"[+] user {username} exists.")
            self.positive_log(f"[+] user {username} exists.")
        else:
            print(f"[-] user {username} does not exist")

    def domain_users_enumerator_threads(self):
        threads = []
        print("\n[*] User Accounts Enumeration\n" + "-" * 30)
        for username in self.users:
            thread = threading.Thread(target=self.user_thread, args=[username])
    ...
    def computer_accounts_enumerator_threads(self):
        threads = []
        print("\n[*] Computer Accounts Enumeration\n" + "-" * 30)
        for computer_account in self.computers:
            thread = threading.Thread(
                target=self.computer_thread, args=[computer_account]
            )
            threads.append(thread)
            thread.start()
    ...
    def computer_thread(self, computer_account: str):
        if computer_account[-1] != "$":
            ...
            if self.domain_info_flag:
                ...
                response = hDsrGetDcNameEx2(
                    self.dce, NULL, computer_account, 0x1000, NULL, NULL, NULL, 0
                )

            if response is not None and response["ErrorCode"] == 0:
                print(f"[+] computer account {computer_account} exists.")
                self.positive_log(f"[+] computer account {computer_account} exists.")
            else:
        ...
        for computer_account in self.computers:
            if computer_account[-1] != "$":
                ...
                if response is not None and response["ErrorCode"] == 0:
                    print(f"[+] computer account {computer_account} exists.")
                    self.positive_log(
                        f"[+] computer account {computer_account} exists."
                    )
                else:
                    print(f"[-] user {computer_account} does not exist")
    ...
    def run(self):
        ...
        try:
            if self.users is not None:
                ...
                self.domain_users_enumerator_threads()
                self.dce.disconnect()
                self.dce = None
        except Exception as e:
            if self.dce is not None:
        ...
        try:
            if self.computers is not None:
                ...
                self.computer_accounts_enumerator_threads()
                self.dce.disconnect()
                self.dce = None
        except Exception as e:
            if self.dce is not None:
